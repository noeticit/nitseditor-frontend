<template>
    <div class="mt-10 px-8 font-bold">
        <div v-for="(item,index) in menus" class="mt-5 " @mouseover="Visible = index" @mouseleave="Visible = index" @keydown.enter="Visible = !Visible">
            <router-link class="inline-flex items-center justify-between px-2 py-1 font-medium  transition-all duration-500 rounded-md focus:outline-none focus:text-brand-900 sm:focus:shadow-outline" :to="item.link">
                <span class="flex-shrink-0 tracking-widest text-gray-700 text-sm uppercase">{{item.name}}</span>
            ...
            <transition enter-active-class="transition duration-300 ease-out transform" enter-class="-translate-y-3 scale-95 opacity-0" enter-to-class="translate-y-0 scale-100 opacity-100" leave-active-class="transition duration-150 ease-in transform" leave-class="translate-y-0 opacity-100" leave-to-class="-translate-y-3 opacity-0">
                <div v-show="Visible === index" class="absolute pt-2">
                    <div class="relative py-1 bg-white border border-gray-200 rounded-md shadow-xl" v-for="ele in item.children">
                        ...
                        <div class="relative">
                            <a href="#" class="block w-full px-4 py-2 font-medium text-gray-700 whitespace-no-wrap hover:bg-gray-100 focus:outline-none hover:text-gray-900 focus:text-gray-900 transition duration-300 ease-in-out">{{ele.name}}</a>
                        </div>
                    </div>
                </div>
            </transition>
