<template>
    <dashboard-layout-one active="model">
        <base-bread-crumb-one name="Edit Model" :breadcrumbs="breadcrumbs"></base-bread-crumb-one>
            ...
            <div class="m-2 w-full  overflow-y-auto h-screen">
                <div class="bg-white mb-32 rounded-lg shadow-lg mt-1 min-h-full jsutify-center">
                    <div class="flex justify-between border-solid border-b-2 p-2 text-center items-center">
                        <div class="text-left ml-4 mt-2 mb-2 text-xl antialiased leading-snug text-gray-900 tracking-normal font-medium font-sans"> Model Edit </div>
                    </div>
                    <div class="p-4">
                            ...
                            <div class="m-2 w-full">
                                ...
                                <div class="flex m-4 w-full">
                                    <div class="inline-flex mt-10 items-center rounded-lg py-2 px-6 bg-teal-700">
                                        <span class="text-center text-base antialiased tracking-tight font-sans text-white cursor-pointer" @click.prevent="update">Update</span>
                                    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            fetchData() {
                this.$api.get('/nits-system-api/model/'+this.$route.params.edit,
                ).then(response => {
                    if (response.status === 200)
                    {
                        this.editDetails = response.data.data
                        this.model_name = this.editDetails.name
                    }
                })
            },
            update() {
                this.$api.update('/nits-system-api/model/'+this.$route.params.edit,
                    {
